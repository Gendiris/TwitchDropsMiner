            ...
            while True:
                try:
                    ...
                    if should_reload or exceeded:
                        ...
                        logger.info(log_msg)
                    service.state_store.record_watchdog(
                        state=state_name or "unknown",
                        idle_min=round(diff_minutes, 1) if diff_minutes is not None else None,
                        threshold_min=round(threshold_minutes, 1),
                        consecutive=consecutive_for_log,
                        action=action,
                    )
